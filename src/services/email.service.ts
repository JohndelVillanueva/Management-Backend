  ...
  try {
    ...
    while (retries > 0) {
      try {
        ...
        console.log('[Email Service] Gmail response:', {
          ...
          response: ((info as any).response ? (info as any).response.substring(0, 100) + '...' : '')
        });
        return;
      } catch (error) {
        retries--;
        ...
        console.warn(`[Email Service] Gmail send failed, ${retries} retries left. Error:`, (error as any).message);
    ...
    if (error instanceof Error) {
      if (error.message.includes('Invalid login')) {
        throw new Error('Gmail authentication failed. Please check your SMTP credentials.');
      }
      if (error.message.includes('Message rejected')) {
        throw new Error('Gmail rejected the email. Please check your account limits or content.');
      }
    }
    
    throw new Error(`Failed to send verification to ${email}. Please try again later.`);
  }
}
